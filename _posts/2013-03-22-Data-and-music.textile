...
One danger is that disonance (e.g. when the difference in pitch between mapped data points becomes a flattened 5th) might make the relationship sound 'bad' when it could be a indicating a positive relationship in the data. The representation of sound already has a 'meaning' other than the value we're given it for the data. It would probably make sense to map the data points to some other kind of filter that's applied to a constant motif instead to try to mitigate this.
